...
import (
	...
	"ks-tool.dev/horchestra/pkg/apiserver"
	"ks-tool.dev/horchestra/pkg/authn"
	"ks-tool.dev/horchestra/pkg/authz"
	"ks-tool.dev/horchestra/pkg/config"
	"ks-tool.dev/horchestra/pkg/nodeapi"
	"ks-tool.dev/horchestra/pkg/nodeserver"
...
// authentication, authorization and the admission chain over BoltDB storage.
func init() {
	cfg := config.Default()
	cmd := &cobra.Command{
		Use:   "controller",
...
func runController(cfg config.Config) error {
	...
	svc := service.New(store, admission.DefaultChain(store))
	...
	// The node-agent transport is a gRPC bi-directional stream (pkg/nodeserver),
	// served on the same TLS port as the REST API: an HTTP/2 request carrying
	// application/grpc is dispatched to the gRPC server, everything else to the
	// REST mux. The gRPC handler reads the node's identity from the mTLS peer
	// certificate, so it needs no auth middleware of its own. It also backs
	// `kubectl logs`, streaming an app's logs from the node it runs on.
	nodes := nodeserver.New(svc)
	grpcServer := grpc.NewServer()
	nodeapi.RegisterNodeServiceServer(grpcServer, nodes)

	srv := apiserver.New(svc, v1.BaseResources, cfg.NodeReadyTimeoutDuration(), nodes,
		apiserver.AuditID,
...
// serverTLS builds the TLS config from the resolved serving material; a
// configured client CA enables client-cert (mTLS) verification.
