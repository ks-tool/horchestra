...
// Casbin authorizes requests with a Casbin enforcer whose policy is projected
// from the declarative rbac.ks-tool.dev/v1 Role/RoleBinding objects. Unlike the
// live RBAC authorizer it does not query storage per request: it compiles the
// policy once (LoadFromStore) and refreshes it on every change (Watch).
type Casbin struct {
	adminGroups []string
	mu          sync.RWMutex
	e           *casbin.Enforcer
...
func (c *Casbin) Watch(ctx context.Context, store storage.Storage) error {
	roles, err := store.Watch(ctx, v1.RoleResource.GVK)
	if err != nil {
		return err
	}
	bindings, err := store.Watch(ctx, v1.RoleBindingResource.GVK)
	if err != nil {
		return err
	}
	for {
...
func rulesFromStore(ctx context.Context, store storage.Storage) ([][]string, error) {
	bindings, err := store.List(ctx, v1.RoleBindingResource.GVK)
	if err != nil {
		return nil, err
	}
	...
	for i := range bindings.Items {
		...
		if !decodeSpec(&bindings.Items[i], &rb) || rb.RoleRef.Kind != "Role" {
			continue
		}
		role, err := store.Get(ctx, v1.RoleResource.GVK, rb.RoleRef.Name)
		if err != nil {
			continue
		}
		var rs v1.RoleSpec
		if !decodeSpec(role, &rs) {
			continue
		}
...
func subjectString(s v1.Subject) string {
	switch s.Kind {
	case "User":
		return "user:" + s.Name
	case "Group":
