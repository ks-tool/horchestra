...
func (c *Config) resolveTLS(deriveAddr bool) error {
	...
	case len(c.AuthConfig) > 0:
		kc, err := kubeconfig.Load(c.AuthConfig)
		if err != nil {
			return err
		}
		...
		c.certPEM, c.keyPEM, c.caPEM = user.ClientCertificateData, user.ClientKeyData, cluster.CertificateAuthorityData
		if deriveAddr && len(cluster.Server) > 0 {
			if u, err := url.Parse(cluster.Server); err == nil && len(u.Host) > 0 {
				c.Addr = u.Host
			}
		}
