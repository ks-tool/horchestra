...
// imageSpec reads the image tagged tag from the shared unpacked OCI layout and
// returns the ordered unpacked layer directories to overlay-mount plus the image
// config (entrypoint/cmd/env/user/workdir) needed to run it.
func imageSpec(ctx context.Context, layoutPath, tag string) ([]string, ocispecv1.ImageConfig, error) {
	// Open, not New: a read must never CREATE the layout. New materialises an empty
	// layout when the path is absent — and, without the Unpack option, in tar mode.
	// The reconciler probes Spec before pulling (the reboot fast path), so on a fresh
	// node that New would create a tar-mode layout, poisoning the subsequent
	// unpack-mode Pull: it reopens the existing (tar) layout, stores compressed blobs
	// instead of unpacked layer directories, and LayerDirs then fails "not in unpack
	// mode". Open fails cleanly when the layout is absent — the reconciler's signal
	// to pull — with no side effect. Resolve matches tag against the ref-name
	// annotation, so it is passed per-call rather than baked into the layout ref.
	ref, err := reference.Parse(reference.OciScheme.String() + layoutPath)
	if err != nil {
	...
	l, ok := repo.(*ocilayout.Layout)
	if !ok {
		return nil, ocispecv1.ImageConfig{}, fmt.Errorf("%s is not an OCI layout", layoutPath)
	}
	imgRef := reference.Reference{Ref: tag}
	dirs, err := l.LayerDirs(ctx, imgRef)
	if err != nil {
		return nil, ocispecv1.ImageConfig{}, err
	}
	img, err := imageConfig(ctx, l, imgRef)
	if err != nil {
		return nil, ocispecv1.ImageConfig{}, err
	}
	return dirs, img.Config, nil
}
