...
func TestNewTableEmptyKeepsColumns(t *testing.T) {
	...
	tbl, err := newTable(v1.ApplicationResource.GVK, nil, defaultNodeReadyTimeout)
	if err != nil {
		t.Fatal(err)
	}
...
func TestNewTableRows(t *testing.T) {
	...
	tbl, err := newTable(v1.ApplicationResource.GVK, []unstructured.Unstructured{app}, defaultNodeReadyTimeout)
	if err != nil {
		t.Fatal(err)
	}
...
func TestNodeTableColumns(t *testing.T) {
	...
	n := v1.Node{
		...
		Status: v1.NodeStatus{
			Capacity:  v1.ResourceAmounts{CPU: resource.MustParse("8"), Memory: resource.MustParse("16Gi")},
			Allocated: v1.ResourceAmounts{CPU: resource.MustParse("2"), Memory: resource.MustParse("8Gi")},
	...
	tbl, err := newTable(v1.NodeResource.GVK, []unstructured.Unstructured{u}, defaultNodeReadyTimeout)
	if err != nil {
		t.Fatal(err)
	}
...
func TestNodeMemGiB(t *testing.T) {
	...
	n := v1.Node{
		...
		Status: v1.NodeStatus{
			Capacity:  v1.ResourceAmounts{Memory: resource.MustParse("8130904Ki")},
			Allocated: v1.ResourceAmounts{Memory: resource.MustParse("512Mi")},
		},
	}
...
func TestPersistentVolumeColumns(t *testing.T) {
	pv := v1.PersistentVolume{
		TypeMeta:   v1.PersistentVolumeResource.TypeMeta(),
		ObjectMeta: metav1.ObjectMeta{Name: "data"},
		Spec:       v1.PersistentVolumeSpec{Size: resource.MustParse("10Gi"), Node: "sav-01"},
	}
	...
	tbl, err := newTable(v1.PersistentVolumeResource.GVK, []unstructured.Unstructured{u}, defaultNodeReadyTimeout)
	if err != nil {
		t.Fatal(err)
	}
	cols := []string{}
...
func TestNodeReadyStatus(t *testing.T) {
	cases := map[string]v1.NodeStatus{
	...
	for name, st := range cases {
		n := v1.Node{TypeMeta: v1.NodeResource.TypeMeta(), ObjectMeta: metav1.ObjectMeta{Name: name}, Status: st}
		data, _ := json.Marshal(n)
		u := unstructured.Unstructured{}
		if err := u.UnmarshalJSON(data); err != nil {
			t.Fatal(err)
