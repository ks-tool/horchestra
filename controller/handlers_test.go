...
// TestClusterScopedRejectsBodyNamespace: a cluster-scoped Kind has no :namespace in its route,
// so a client-supplied metadata.namespace used to pass straight through. Storage keys the object
// by its own metadata while every Get/Update/Delete route addresses a cluster-scoped Kind with an
// empty namespace — so the record existed, showed up in every List, and could never be addressed
// or deleted again.
func TestClusterScopedRejectsBodyNamespace(t *testing.T) {
	s := newHandlerServer(t)
	const base = "/apis/test.horchestra.io/v1/widgets"

	w := doReq(t, s, http.MethodPost, base, "application/json",
		`{"metadata":{"name":"ghost","namespace":"smuggled"},"spec":{"image":"nginx"}}`)
	if w.Code != http.StatusBadRequest {
		t.Fatalf("POST with a namespace on a cluster-scoped kind = %d, want 400 (body=%s)", w.Code, w.Body.String())
	}

	// The object must not exist, in any spelling.
	if w := doReq(t, s, http.MethodGet, base+"/ghost", "", ""); w.Code != http.StatusNotFound {
		t.Fatalf("the rejected object is addressable: GET = %d", w.Code)
	}
	list := doReq(t, s, http.MethodGet, base, "", "")
	if strings.Contains(list.Body.String(), "ghost") {
		t.Fatalf("the rejected object appears in the collection: %s", list.Body.String())
	}

	// A cluster-scoped create without a namespace still works, and stays addressable.
	if w := doReq(t, s, http.MethodPost, base, "application/json",
		`{"metadata":{"name":"real"},"spec":{"image":"nginx"}}`); w.Code != http.StatusCreated {
		t.Fatalf("an ordinary cluster-scoped create = %d, want 201 (body=%s)", w.Code, w.Body.String())
	}
	if w := doReq(t, s, http.MethodGet, base+"/real", "", ""); w.Code != http.StatusOK {
		t.Fatalf("GET of the created object = %d, want 200", w.Code)
	}
	if w := doReq(t, s, http.MethodDelete, base+"/real", "", ""); w.Code != http.StatusOK {
		t.Fatalf("DELETE of the created object = %d, want 200", w.Code)
	}
}
