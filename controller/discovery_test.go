...
func TestAPIGroupList_MultiVersionGroup(t *testing.T) {
	...
	// metrics.k8s.io is always advertised beside whatever the scheme holds: it is derived
	// rather than stored, so nothing else would put it in the index — and without the index
	// entry kubectl never looks for the API and `kubectl top` reports it unavailable.
	var g *metav1.APIGroup
	var sawMetrics bool
	for i := range list.Groups {
		switch list.Groups[i].Name {
		case discGroup:
			g = &list.Groups[i]
		case "metrics.k8s.io":
			sawMetrics = true
	...
	if !sawMetrics {
		t.Error("the metrics group is missing from discovery; kubectl top would not find it")
	}
	if g == nil {
		t.Fatalf("group %q is absent from %+v", discGroup, list.Groups)
	}
