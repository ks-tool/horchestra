...
func TestApplication_Lifecycle(t *testing.T) {
	...
	if created.Spec.Image != "postgres:16" || created.Spec.Placement.NodeName != "node-1" {
	...
	if patched.Spec.Placement.NodeName != "node-1" {
		t.Errorf("merge patch dropped nodeName = %q", patched.Spec.Placement.NodeName)
	}

	// Delete returns a success Status. The object does NOT vanish: this app is placed, so it
	// carries the node-teardown finalizer and the delete is a REQUEST — it stays, stamped, until
	// the node that runs the workload reports the workload gone. That is the whole point of the
	// two phases: an object erased on the spot meant a node learned of the deletion by absence,
	// and a workload that would not stop disappeared from the API while its unit ran on.
	...
	code, body = s.get(appPath("db"))
	if code != http.StatusOK {
		t.Fatalf("get after delete = %d, want the object to still be there while it is torn down", code)
	}
	var deleting corev1.Application
	decode(t, body, &deleting)
	if deleting.DeletionTimestamp == nil {
		t.Error("the object survived the delete without being stamped, so nothing marks it as going")
	}
	if !slices.Contains(deleting.Finalizers, corev1.FinalizerNodeTeardown) {
		t.Errorf("finalizers = %v, want the node-teardown hold", deleting.Finalizers)
	}

	// An unplaced app has nothing to wait for — no node holds a workload for it — so its delete
	// is the erase it always was. Without this the finalizer would be a trap: an object waiting
	// on a report nobody is going to send.
	if code, body := s.create(appPath(""), json.RawMessage(
		`{"apiVersion":"horchestra.io/v1","kind":"Application","metadata":{"name":"unplaced"},`+
			`"spec":{"image":"reg/x:v1"}}`)); code != http.StatusCreated {
		t.Fatalf("create unplaced = %d, body=%s", code, body)
	}
	if code, body := s.del(appPath("unplaced")); code != http.StatusOK {
		t.Fatalf("delete unplaced = %d, body=%s", code, body)
	}
	if code, _ := s.get(appPath("unplaced")); code != http.StatusNotFound {
		t.Errorf("get after deleting an unplaced app = %d, want 404", code)
	}
}
