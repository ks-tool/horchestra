...
func TestApplication_Watch(t *testing.T) {
	...
	// A delete on a PLACED app is a request, not an erase, so what a watcher sees first is the
	// object being stamped — MODIFIED carrying a deletionTimestamp. DELETED comes only when the
	// node that runs the workload reports it gone and the last finalizer is released; there is
	// no node in this test, so this is where the sequence stops.
	if code, _ := s.del(appPath("db")); code != http.StatusOK {
		t.Fatalf("delete failed")
	}
	e := waitEvent(t, dec)
	if e.Type != "MODIFIED" {
		t.Fatalf("event 3 = %q, want MODIFIED — a delete is watched as the stamp that starts it", e.Type)
	}
	if got := watchedApp(t, e); got.DeletionTimestamp == nil {
		t.Error("the watched object carries no deletion timestamp, so a watcher cannot tell it is going")
	}
}
