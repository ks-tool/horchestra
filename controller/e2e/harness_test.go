...
func startServer(t *testing.T) *testServer {
	...
	api := apiserver.New(sch, service.New(store, sch, admission.DefaultChain(nil, nil)))
	api.EmulatePodsAPI()
...
func nsResourcePath(group, version, namespace, plural, name string) string {
	p := "/apis/" + group + "/" + version + "/namespaces/" + namespace + "/" + plural
	if name != "" {
		p += "/" + name
	}
	return p
...
// appPath addresses Applications in the "default" namespace (Application is namespaced).
func appPath(name string) string {
	return nsResourcePath("horchestra.io", "v1", "default", "applications", name)
}
func nodePath(name string) string { return resourcePath("horchestra.io", "v1", "nodes", name) }

// pvPath addresses PersistentVolumes in the "default" namespace (PV is namespaced).
func pvPath(name string) string {
	return nsResourcePath("horchestra.io", "v1", "default", "persistentvolumes", name)
}
func rolePath(name string) string {
	return nsResourcePath("rbac.horchestra.io", "v1", "default", "roles", name)
}
func rbPath(name string) string {
	return nsResourcePath("rbac.horchestra.io", "v1", "default", "rolebindings", name)
}
...
func newApp(name, node, image string) *corev1.Application {
	return &corev1.Application{
		...
		Spec:       corev1.ApplicationSpec{Image: image, Placement: corev1.Placement{NodeName: node}},
	}
}
