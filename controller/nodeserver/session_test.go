...
func (f *fakeController) Get(ctx context.Context, m types.ObjectMeta) (types.Object, error) {
	obj, err := f.store.Get(ctx, m)
	// The real service maps the storage sentinels to API errors; the server reads NotFound on
	// this path to decide whether the node is still a fleet member.
	return obj, mapStorageErr(gvkOfMeta(m), err)
}

func gvkOfMeta(m types.ObjectMeta) schema.GroupVersionKind {
...
func (f *fakeController) Create(ctx context.Context, gvk schema.GroupVersionKind, data []byte, _ string) (types.Object, error) {
...
func (f *fakeController) Update(ctx context.Context, gvk schema.GroupVersionKind, data []byte, _, _ string) (types.Object, error) {
	obj, err := f.decode(gvk, data)
	if err != nil {
		return nil, err
	}
...
func (f *fakeController) UpdateSubresource(ctx context.Context, gvk schema.GroupVersionKind, subresource string, data []byte, _ string) (types.Object, error) {
	obj, err := f.decode(gvk, data)
	if err != nil {
		return nil, err
	}
	out, err := f.store.UpdateSubresource(ctx, subresource, obj)
	return out, mapStorageErr(gvk, err)
}

// Delete erases straight from the store. The real service is two-phase — an object holding a
// finalizer is stamped rather than erased — but by the time the node-server calls this the
// finalizer it was waiting on is already gone, so the erase is what these tests must observe.
func (f *fakeController) Delete(ctx context.Context, m types.ObjectMeta, _ metav1.DeleteOptions) error {
	return mapStorageErr(schema.FromAPIVersionAndKind(m.ApiVersion, m.Kind), f.store.Delete(ctx, m))
}
...
func newHarness(t *testing.T) *harness {
	...
	// The node is a fleet member: desired state is served only while its Node object exists
	// (see push), so an admitted node has one.
	...
	nodeapipb.RegisterNodeServiceServer(gs, srv)
...
func (h *harness) session(t *testing.T, ctx context.Context, cn string, orgs []string) grpc.BidiStreamingClient[nodeapipb.NodeMessage, nodeapipb.ControllerMessage] {
	t.Helper()
	...
	stream, err := nodeapipb.NewNodeServiceClient(conn).Session(ctx)
	if err != nil {
...
func mustCreateApp(t *testing.T, ctl *fakeController, name, node string) {
	t.Helper()
	body := `{"metadata":{"name":"` + name + `"},"spec":{"image":"reg/` + name + `:v1","placement":{"nodeName":"` + node + `"}}}`
...
func mustCreateNode(t *testing.T, ctl *fakeController, name string) {
	t.Helper()
	body := `{"metadata":{"name":"` + name + `"},"spec":{}}`
	if _, err := ctl.Create(context.Background(), corev1.GroupVersion.WithKind("Node"), []byte(body), ""); err != nil {
...
func TestSession_PushesDesiredStateAndPersistsStatus(t *testing.T) {
	...
	if err := stream.Send(&nodeapipb.NodeMessage{Body: &nodeapipb.NodeMessage_Status{
		Status: &nodeapipb.NodeStatus{Node: []byte(nodeJSON)},
	}}); err != nil {
...
func TestSession_StreamsLogs(t *testing.T) {
	...
	send := func(m *nodeapipb.NodeMessage) {
		if err := stream.Send(m); err != nil {
			t.Fatalf("send: %v", err)
		}
	}
	send(&nodeapipb.NodeMessage{Body: &nodeapipb.NodeMessage_LogChunk{LogChunk: &nodeapipb.LogChunk{Id: req.GetId(), Data: []byte("hello\n")}}})
	send(&nodeapipb.NodeMessage{Body: &nodeapipb.NodeMessage_LogChunk{LogChunk: &nodeapipb.LogChunk{Id: req.GetId(), Eof: true}}})
...
func recvLogRequest(t *testing.T, stream grpc.BidiStreamingClient[nodeapipb.NodeMessage, nodeapipb.ControllerMessage]) *nodeapipb.LogRequest {
	t.Helper()
	for {
		msg, err := stream.Recv()
		if err != nil {
