...
func (f fakeLister) List(_ context.Context, m types.ObjectMeta, _ metav1.ListOptions) ([]types.Object, error) {
	...
	case "Namespace":
		for i := range f.namespaces {
			out = append(out, &f.namespaces[i])
		}
	case "PersistentVolume":
		for i := range f.pvs {
			out = append(out, &f.pvs[i])
		}
	case "Service":
...
func mkApp(name, node string, req corev1.ResourceAmounts) corev1.Application {
	return corev1.Application{
		...
		Spec:       corev1.ApplicationSpec{Image: "reg/app:v1", Placement: corev1.Placement{NodeName: node}, Resources: corev1.ResourceRequirements{Requests: req}},
	}
}
...
func TestCapacityCheck(t *testing.T) {
	...
	t.Run("same name in another namespace still counts", func(t *testing.T) {
		other := mkApp("web", "n1", cpu("3"))
		other.Namespace = "team-b"
		c := capacityCheck{lister: fakeLister{
			apps:  []corev1.Application{other},
			nodes: []corev1.Node{mkNode("n1", cap4)},
		}}
		// Creating team-a/web (2) on n1: team-b/web (3) shares the name but is a different object,
		// so it must still count → 3+2=5 > 4. (A name-only exclusion would drop it and admit.)
		app := mkApp("web", "n1", cpu("2"))
		app.Namespace = "team-a"
		if err := c.Validate(ctx, appAttrs(Create, app)); err == nil {
			t.Fatal("a same-named app in another namespace must count against the node")
		}
	})
}
