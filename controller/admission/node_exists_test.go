...
func TestNodeExists(t *testing.T) {
	...
	check := ruleCheck(fakeLister{nodes: nodes}, nodeExistsRule)
	...
	t.Run("missing node rejected even with no requests", func(t *testing.T) {
		...
		err := check.Validate(ctx, appAttrs(Create, mkApp("a", "ghost", corev1.ResourceAmounts{})))
	...
	t.Run("nil lister skips", func(t *testing.T) {
		if err := ruleCheck(nil, nodeExistsRule).Validate(ctx, appAttrs(Create, mkApp("a", "ghost", cpu("1")))); err != nil {
