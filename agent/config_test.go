...
func TestLoadAuthConfig(t *testing.T) {
	...
	if !bytes.Equal(cfg.CertData, cert) {
		t.Errorf("CertData = %q, want %q", cfg.CertData, cert)
	}
	if !bytes.Equal(cfg.KeyData, key) {
		t.Errorf("KeyData = %q, want %q", cfg.KeyData, key)
	}
	if !bytes.Equal(cfg.CAData, ca) {
		t.Errorf("CAData = %q, want %q", cfg.CAData, ca)
	}
