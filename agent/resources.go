...
// whole-core quantity, memory a byte quantity. The agent runs only on Linux, so the
// probes read Linux's /proc and /etc; off Linux those files are absent and each
// probe yields its zero value. Probes are best-effort — a failure leaves that field
// zero. Allocation is derived by the reconciler from running apps, not measured here.
...
// /proc/sys/kernel/osrelease (the same value as `uname -r`), empty off Linux.
